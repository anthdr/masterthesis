...
```{r}
bargraph.CI(x.factor=verbetype, group= prompt,response=attachment, col=c("tomato", "dark blue"), xlab="Verb type (highly mathematically non-skillful group), 2",ylab="NP1 Attachment %",legend=TRUE,  x.leg=3,y.leg=0.8 , cex.leg=1.3, cex.names=1, cex.lab = 1.5, ylim = c(0,1), data=datamath)
```
...
```{r}
bargraph.CI(x.factor=verbetype, group= prompt,response=attachment, col=c("tomato", "dark blue"), xlab="Verb type (highly mathematically non-skillful group, fast equation), 2",ylab="NP1 Attachment %",legend=TRUE,  x.leg=3,y.leg=0.8 , cex.leg=1.3, cex.names=1, cex.lab = 1.5, ylim = c(0,1), data=datamath.rapide)
```
...
```{r}
bargraph.CI(x.factor=verbetype, group= prompt,response=attachment, col=c("tomato", "dark blue"), xlab="Verb type (highly mathematically non-skillful group, slow equation), 2",ylab="NP1 Attachment %",legend=TRUE,  x.leg=3,y.leg=0.8 , cex.leg=1.3, cex.names=1, cex.lab = 1.5, ylim = c(0,1), data=datamath.lent)
```
...
```{r}
df.math
write.csv(df.math, file = "df.nonmath.csv")
```

```{r}
ggplot(data=df.math, aes(x=mean, y=effect)) + theme_bw() +
  ggtitle("Interaction: non-mathematically skillful group") +
