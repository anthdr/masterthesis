...
```{r}
get_prior(attachment ~ vt*pr*slow*eq*hs + (vt*pr*slow*eq*hs|id) + (vt*pr*slow*eq*hs|Item), data=data, family=bernoulli)
...
```{r}
fit <- brm(attachment ~ 1+vt*pr*slow*eq*hs + (1+vt*pr*slow*eq*hs|id) + (1+vt*pr*slow*eq*hs|Item), data=data, family=bernoulli, 
