...
```{r}
get_prior(attachment ~ vt*pr*slow*hs + (vt*pr*slow*hs|id) + (vt*pr*slow*hs|Item), data=data, family=bernoulli)
...
```{r}
fit <- brm(attachment ~ 1+vt*pr*slow*hs + (1+vt*pr*slow*hs|id) + (1+vt*pr*slow*hs|Item), data=data, family=bernoulli, 
