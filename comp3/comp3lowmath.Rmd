...
```{r}
bargraph.CI(x.factor=prompt,response=attachment, col=c("tomato", "dark blue"), xlab="Prime (mathematically non-skillful group), 3",ylab="NP1 Attachment %",legend=TRUE,  x.leg=3,y.leg=0.8 , cex.leg=1.3, cex.names=1, cex.lab = 1.5, ylim = c(0,1), data=datamath)
```
...
```{r}
bargraph.CI(x.factor=prompt,response=attachment, col=c("tomato", "dark blue"), xlab="Prime (mathematically non-skillful group, fast equations), 3",ylab="NP1 Attachment %",legend=TRUE,  x.leg=3,y.leg=0.8 , cex.leg=1.3, cex.names=1, cex.lab = 1.5, ylim = c(0,1), data=datamath.rapide)
```
...
```{r}
bargraph.CI(x.factor=prompt,response=attachment, col=c("tomato", "dark blue"), xlab="Prime (mathematically non-skillful group, slow equation), 3",ylab="NP1 Attachment %",legend=TRUE,  x.leg=3,y.leg=0.8 , cex.leg=1.3, cex.names=1, cex.lab = 1.5, ylim = c(0,1), data=datamath.lent)
```
...
```{r}
df.math
write.csv(df.math, file = "df.lowmath.csv")
```

```{r}
library(ggplot2)
ggplot(data=df.math, aes(x=mean, y=effect)) + theme_bw() +
  ggtitle("Interaction: mathematically non-skillful group") +
