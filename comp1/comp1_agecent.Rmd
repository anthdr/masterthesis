...
data$agcent <- scale(data$age.y, scale=T, center=T)
```

```{r}
get_prior(comp.graph ~ vt*pr*slow*agcent + (vt*pr*slow*agcent|id) + (vt*pr*slow*agcent|Item), data=data, family=bernoulli)
...
```{r}
fit <- brm(comp.graph ~ 1+vt*pr*slow*agcent + (1+vt*pr*slow*agcent|id) + (1+vt*pr*slow*agcent|Item), data=data, family=bernoulli, 
...
```{r}
df.overall
write.csv(df.overall, file = "df.overall.agecent.csv")
```
